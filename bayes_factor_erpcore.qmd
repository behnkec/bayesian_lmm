...
Prior for b N(0,10) -> Estimated Bayes factor in favor of x1 over x2: 6027.14818
...
```{r}
mod_erpcore_dis_bf_0_1 <- brm(N170 ~ 1 + (1 + f_c | participant_id),
                            data = trials_cond, 
...
mod_erpcore_dis_bf_0_2 <- brm(N170 ~ 1 + (1 + f_c | participant_id),
                            data = trials_cond, 
...
for (i in 1:length(prior_b_sd)) {
  ...
  mod_1 <- brm(N170 ~ 1 + f_c + (1 + f_c | participant_id),
             data = trials_cond, 
             prior = c(prior(normal(0, 10), class = Intercept),
  ...
  mLL_linear_dis_1 <- bridge_sampler(mod, silent = TRUE)
  
  BF_1 <- c(BF_1, bayes_factor(mLL_linear_dis_1, mLL_null_dis_1)$bf)
}
...
for (i in 1:length(prior_b_sd)) {
  ...
  mod_2 <- brm(N170 ~ 1 + f_c + (1 + f_c | participant_id),
             data = trials_cond, 
             prior = c(prior(normal(0, 20), class = Intercept),
                       set_prior(paste0("normal(0,", pbsd, ")"), class = b, coef = f_cface),
                       prior(normal(0, log(100)), class = Intercept, dpar = sigma),
  ...
  mLL_linear_dis_2 <- bridge_sampler(mod_2, silent = TRUE)
  
  BF_2 <- c(BF_2, bayes_factor(mLL_linear_dis_2, mLL_null_dis_2)$bf)
}


BFs <- tibble::tibble(beta_sd = rep(prior_b_sd, 2), BF = c(BF_1, BF_2), 
...
plot <- ggplot(BFs, aes(x = beta_sd, y = BF, color = Prior)) +
  geom_line() +  # Linie zeichnen
  geom_point() +
  scale_y_log10(
    limits = c(0.01, max(BFs$BF)),  # Begrenzung der y-Achse

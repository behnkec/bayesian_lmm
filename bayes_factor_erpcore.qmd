...
mLL_null_dis <- bridge_sampler(mod_erpcore_dis_bf_0, silent = TRUE)
...
for (i in 1:length(prior_b_sd)) {
  ...
  mod <- brm(N170 ~ 1 + f_c + (1 + f_c | participant_id),
             data = trials_cond, 
             prior = c(prior(normal(0, 10), class = Intercept),
  ...
  mLL_linear_dis <- bridge_sampler()
  
  BF_1 <- c(BF_1, bayes_factor(mLL_linear_dis, mLL_null_dis)$bf)
}
...
for (i in 1:length(prior_b_sd)) {
  ...
  BF_2 <- c(BF_2, bayes_factor(mLL_linear_dis, mLL_null_dis)$bf)
}
